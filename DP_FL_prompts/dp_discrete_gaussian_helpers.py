...
def compute_rdp_dgaussian(q, l1_scale, l2_scale, dim, steps, orders):
...
def ddgauss_epsilon(gamma,
  ...
  if rounding:
    l2_sens = rounded_l2_norm_bound(l2_sens * scale, beta, dim) / scale # get rid of the effect of scale. sensitivity is ~Clipping norm + some function(gamma, beta, dim)
    l1_sens = rounded_l1_norm_bound(l2_sens * scale, dim) / scale

  l1_scale = l1_sens / (local_stddev)
  l2_scale = l2_sens / (local_stddev)
  rdp = compute_rdp_dgaussian(q, l1_scale, l2_scale, dim, steps, orders)
...
def ddgauss_params(q,
  ...
  def stddev(x):
    return ddgauss_local_stddev(q, epsilon, l2_clip_norm, x, beta, steps, dim, delta, orders)

  def mod_min(x):
    return k * math.sqrt(rho / dim * l2_clip_norm ** 2 * n_factor +
                         (x**2 / 4.0 * num_datapoints_total_per_round + stddev(x)**2))
...
if __name__ == '__main__':
  gamma, local_stddev = ddgauss_params(900/67200,
                     8,
                     0.01,
                     16,
                     900,
