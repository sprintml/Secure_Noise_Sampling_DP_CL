...
class RobertaPromptForSequenceClassificationWithInfilling(RobertaPreTrainedModel):
    def __init__(self, config):
        ...
        if config.freeze_non_prompt_layers:
    ...
    def forward(
        ...
        if mask_pos is not None:
            # shift the mask_pos by pre_seq_len
            mask_pos = mask_pos.squeeze()  # (batch_size,), a sequence of ints.
            if self.config.shift_mask_pos_by_p_len:
                mask_pos = mask_pos + self.pre_seq_len
...
class RobertaPrefixForSequenceClassificationWithInfilling(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        # Pick the entries that correspond to labels.
        if mask_pos is not None:
            # prefix: don't need to shift since the prefix is added/attached to attention layers
            mask_pos = mask_pos.squeeze()  # (batch_size,), a sequence of ints.
        sequence_mask_output = sequence_output[torch.arange(batch_size), mask_pos]
...
class RobertaPrefixForSequenceClassificationDebug(RobertaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
