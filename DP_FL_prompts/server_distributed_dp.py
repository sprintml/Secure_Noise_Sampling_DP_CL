...
from helper_secagg import (mod_vector_m, get_actual_sentivity, postprocessing_client_gradient,
    postprocessing_client_gradient_distributed,    \
...
def write_metrics(metrics, elapsed_time=0):
    ...
    #     writer.write(
    #         f"{training_args.training_type},{training_args.privacy_engine},{model_args.method_type},{data_args.dataset_name},"
    #         f"{model_args.model_name_or_path},{training_args.per_device_train_batch_size},{training_args.learning_rate},"
    #         f"{privacy_args.target_epsilon},{privacy_args.per_sample_max_grad_norm},{collaboration_args.num_global_server_epochs},"
    #         f"{model_args.pre_seq_len},{metrics['eval_loss']},{metrics['eval_accuracy']},{elapsed_time},{elapsed_time / 3600},"
    #         f"{current_time},{training_args.gradient_accumulation_steps},{privacy_args.target_delta},{training_args.training_type}\n")
...
def aggregate_model_states(args, aggregate_trainer, client_trainers,):
    ...
    gradient_noise, local_gradient_noise = {}, {}
    template = {}
    ...
    for client_id, trainer in client_trainers.items():
        ...
        if privacy_args.use_discrete:
            if privacy_args.distributed_dp:
                client_gradient_update = postprocessing_client_gradient_distributed(client_gradient_update, template, seed_pair,
                                                                        privacy_args)
            else:
                raise NotImplementedError("this script is for distributed DP")
        else:
            raise NotImplementedError("this script is for discrete gaussian distributed DP")
        if privacy_args.distributed_dp:
            if privacy_args.noise_multiplier > 0 and privacy_args.per_sample_max_grad_norm > 0:
                local_gradient_noise = get_noise(privacy_args.noise_multiplier,
                                           get_actual_sentivity(privacy_args),
                                           client_gradient_update.device,
                                           client_gradient_update.size(),
                                           client_gradient_update.numel(),
                                           client_gradient_update.dtype,
                                           privacy_args, privacy_args.discretization_granularity
                                           )
                client_gradient_update = add_two_model_states(client_gradient_update.unsqueeze(0),
                                                              local_gradient_noise.unsqueeze(0), param_names=trainable_params)
                client_gradient_update = mod_vector_m(client_gradient_update, args=privacy_args)
    ...
    assert aggregate_gradient.size(0) == 1
    aggregate_gradient_plus_noise = aggregate_gradient.sum(0)
...
if __name__ == '__main__':
    ...
    # privacy_args.distributed_dp = 1
    # privacy_args.num_colluding_clients = 9
    ...
    # if os.path.isdir(collaboration_args.global_output_dir) and collaboration_args.global_output_dir and not collaboration_args.global_overwrite_output_dir:
    #     last_checkpoint = utils.find_latest_checkpoint(collaboration_args.global_output_dir, "aggregate_", ".pt")
    #     if last_checkpoint is None and len(os.listdir(collaboration_args.global_output_dir)) > 0:
    #         raise ValueError(
    #             f"Output directory ({collaboration_args.global_output_dir}) already exists and is not empty. "
    #             "Use --overwrite_output_dir to overcome."
    #         )
    #     elif last_checkpoint is not None and collaboration_args.global_resume_from_checkpoint is None:
    #         logger.info(
    #             f"Checkpoint detected, resuming training at {last_checkpoint}. To avoid this behavior, change "
    #             "the `--output_dir` or add `--overwrite_output_dir` to train from scratch."
    #         )

    # Split and set the client parameters here:
    # Individual clients:
    # - privacy engine: clipping_norm is set, but noise_multiplier must be 0
    # - learning rate = 1.0 —> this way the update after one step (state_after - state_init) * client batch_size would be the sum of all gradient —> override by us into client_training_args[client_id]
    # - lr_scheduler = None
    # - optimizer = SGD without any momentum or decay
    #
    # Aggregate:
    # - privacy_engine = None
    # - learning rate set by us in the training_args
    # - set param.grad = [sum of all clients’ (state_after - state_init) * client_batch_size] / aggregate batch size
    # - the aggregate batch size is set to be the sum of all client_batch_size
    # - lr_scheduler set by us
    # - optimizer = AdamW set by us
        ...
        if privacy_args.use_discrete:
            if privacy_args.distributed_dp:
                privacy_args.discretization_granularity, privacy_args.noise_multiplier = ddgauss_params(collaboration_args.sampling_rate, #
                                                                                        ...
                                                                                        collaboration_args.num_clients - privacy_args.num_colluding_clients,
                ...
                privacy_args.discretization_granularity, privacy_args.noise_multiplier = ddgauss_params(
            ...
            print(f"distributed_dp: {privacy_args.distributed_dp}")
            print(f"num_colluding_clients: {privacy_args.num_colluding_clients}")
            print(f"discretization granularity gamma: {privacy_args.discretization_granularity}")
    ...
    if training_args.do_train:
        ...
        for global_step in range(last_checkpoint, collaboration_args.num_global_server_steps):
            ...
            for client_id in range(collaboration_args.num_clients):
                ...
                if training_args.training_type == 'private' and collaboration_args.private_type == 'bound_datapoint':
                    if training_args.privacy_engine == 'private_transformers':
                        # print('before model: ', trainer.model.state_dict()['classifier.weight'])
                        # print(train_indices)
                        trainer.named_params_grad_sample = None
                        train_result = trainer.train()
                        # trainer.model = trainer.model.to('cpu')
                        # print('after model: ', trainer.model.state_dict()['classifier.weight'])
                    else:
            ...
            if (global_step + 1) % collaboration_args.save_freq_aggregate == 0 or (global_step + 1) == collaboration_args.num_global_server_steps:
                ...
                if privacy_args.use_discrete:
                    # eps, delta = compute_epsilon_discrete((global_step + 1), collaboration_args.sampling_rate,
                    #                                       privacy_args.noise_multiplier, privacy_args.target_delta,
                    #                                       sensitivity=get_actual_sentivity(privacy_args))
