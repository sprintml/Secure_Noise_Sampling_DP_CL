...
from model.sequence_classification import (
    ...
    RobertaPrefixForSequenceClassificationDebug,
    ...
    RobertaPrefixForSequenceClassificationWithInfilling,
...
PREFIX_MODELS_INFILL = {
    "roberta": {
        TaskType.SEQUENCE_CLASSIFICATION: RobertaPrefixForSequenceClassificationWithInfilling,
    }
}
...
def get_model(model_args, task_type: TaskType, config: AutoConfig, fix_bert: bool = False):
    if model_args.method_type == 'prefix':
        ...
        # config.freeze_non_prompt_layers = model_args.freeze_non_prompt_layers
        ...
        config.pre_seq_len = model_args.pre_seq_len
        config.freeze_non_prompt_layers = model_args.freeze_non_prompt_layers
        config.shift_mask_pos_by_p_len = model_args.shift_mask_pos_by_p_len
    ...
    elif model_args.method_type == 'prefix-infill':
        print('prefix-infill task!')
        config.hidden_dropout_prob = model_args.hidden_dropout_prob
        config.pre_seq_len = model_args.pre_seq_len
        config.prefix_projection = model_args.prefix_projection
        config.prefix_hidden_size = model_args.prefix_hidden_size
        ...
        config.freeze_non_prompt_layers = model_args.freeze_non_prompt_layers
        model_class = PREFIX_MODELS_INFILL[config.model_type][task_type]
        model = model_class.from_pretrained(
            model_args.model_name_or_path,
            config=config,
            revision=model_args.model_revision,
